...
public class GameTree {
    ...
    int findValue() {
        ...
        if (this.root == null) {
            ...
            TreeTraversal(this.root, Player.ONE);
        }
        return this.root.value;
    }

    void TreeTraversal(TreeNode node, Player player) {
        if (node.leaf) {
            return; // Since there's no "left" or "right" demarcation, the only way to terminate base case
            // is checking if the current node is a leaf node
        }

        if (player == Player.ONE) { // Search for maximum value amongst child nodes
            int largest = Integer.MIN_VALUE;
            for (int i = 0; i < node.children.length; i++) {
                TreeTraversal(node.children[i], other(player));
            }
            for (int i = 0; i < node.children.length; i++) {
                if (node.children[i].value > largest) {
                    largest = node.children[i].value;
                }
            }

            node.value = largest;
        } else if (player == Player.TWO) { // Search for minimum value amongst child nodes
            int smallest = Integer.MAX_VALUE;
            for (int i = 0; i < node.children.length; i++) {
                TreeTraversal(node.children[i], other(player));
            }
            for (int i = 0; i < node.children.length; i++) {
                if (node.children[i].value < smallest) {
                    smallest = node.children[i].value;
                }
            }
        ...
        /*else {
            for (int i = 1; i < this.root.children.length; i++) {
                TreeTraversal(this.root.children[i]);
                if (this.root.children[i].leaf) {
                    node.value = root.children[i].value; // If it's a leaf node
                }
            }
            int maximum = this.root.children[0].value;
            for (int i = 1; i < this.root.children.length; i++) {
                if (this.root.children[i].value > maximum) {
                    maximum = this.root.children[i].value;
                }
            }
            node.value = maximum;
        }*/
